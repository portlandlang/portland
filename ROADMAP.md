...
**Next:** inference's last rung, 3c contracts ([#9](https://github.com/portlandlang/portland/issues/9), ADR 0040 §1) — a def's parameters typed by what its body demands, sharpened by caller flow, so the checks reach inside defs where today every parameter is unknown; and the builtin method table ([#88](https://github.com/portlandlang/portland/issues/88)), which gives shape 2 its builtin receivers. The 2026-09-22 session ratified the error voice ([ADR 0047](docs/adr/0047-2026-09-22-the-error-voice.md)) and built it the same day — one renderer, the five refusals, arity — then 3d: the seven narrowing forms, and ADR 0035's tripwire fired. The transitional-tooling trio (#36/#57/#58) is deliberately parked while the language grows.
