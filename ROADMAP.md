...
**Next:** the `%` zoo (#29), smart constructors (job 3 of [#61](https://github.com/portlandlang/portland/issues/61)), or the parallel runtime that ADR 0029's serial oracle now waits for (#12/#13 territory). The 2026-07-27 wave landed ADRs 0025–0029 decided *and* built — write-site `return`, `inspect()`, typed results with `!`, traits, and serial `together` — the decisions by competing draft PRs, the roads not taken recorded in closed siblings. The transitional-tooling trio (#36/#57/#58) is deliberately parked while the language grows.
