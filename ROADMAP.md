...
**Stage 0 is done and Stage 1 has begun.** The Rust seed interprets a real slice of Portland. The compiler — `lexer.pdx`, `parser.pdx`, `evaluator.pdx`, plus the two walkers the seed will never have, `checker.pdx` and `inference.pdx` — is Portland written in Portland: the parser parses the whole compiler including itself, the evaluator matches the seed byte-for-byte, the checker refuses only what it can disprove, and inference computes types it does not yet act on. There is no compiler emitting binaries yet.

**Next:** the two doors the checker's reach now waits on — the builtin method table ([#88](https://github.com/portlandlang/portland/issues/88)), which gives shape 2 and the call-site contract check their builtin receivers, and a whole-program door ([#94](https://github.com/portlandlang/portland/issues/94)), which lets caller flow sharpen parameters and lets every check see the defs and structs a required file declares. ADR 0040's ladder is otherwise climbed: the 2026-09-22/23 sessions ratified the error voice ([ADR 0047](docs/adr/0047-2026-09-22-the-error-voice.md)) and the contract rulings ([ADR 0048](docs/adr/0048-2026-09-23-contracts-the-rulings.md)) and built both the same day — one renderer, the five refusals, arity, narrowing, ADR 0035's tripwire, contracts in the dump, and the call-site refusal. The transitional-tooling trio (#36/#57/#58) is deliberately parked while the language grows.
