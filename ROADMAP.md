...
**Next:** inference's remaining ladder ([#9](https://github.com/portlandlang/portland/issues/9)) — 3c contracts and 3d narrowing, which unlock the maybe-used-as-plain refusal (ADR 0047 shape 3) and ADR 0035's tripwire on coverage; and a builtin method table, which gives shape 2 its builtin receivers and the seed a runtime row that can tell a missing method from wrong arguments. The 2026-09-22 session ratified the error voice ([ADR 0047](docs/adr/0047-2026-09-22-the-error-voice.md)) and built it the same day: one renderer, and the first inference-backed refusals — conditions, operators, struct methods, lying annotations, arity. The transitional-tooling trio (#36/#57/#58) is deliberately parked while the language grows.
