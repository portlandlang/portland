...
- [ ] #31 — markdown linting (`mdl`), and whether to adopt `overcommit` for hooks
- [ ] #32 — a coarse performance tripwire so a slow test can't hide (tune after #34)
- [ ] #33 — audit the guidance docs (AGENT/DESIGN/ROADMAP/STAGE0): one source of truth per fact
- [ ] #34 — make the self-parse test fast via RC-exact `<<` (root cause; ~200M array copies today)
