...
- **Command calls** at statement position (`puts "hello"`, `shout word, other`), **bare zero-argument calls** (`ready?`, `pdx`) anywhere, and **dot calls with arguments** ([ADR 0032](../adr/0032-2026-08-11-dotted-calls-take-paren-less-arguments.md)): `expect(x).to eq(y)`, `words.join "-"`, `"portland".slice 0, 4`. Adjacency nests to the innermost call and commas make siblings — Ruby's parse for these forms, adopted whole. One line stays uncrossed on purpose: a *bare* command as an argument (`.to eq y`, `f g x`) does not parse; the matcher keeps its parens.
...
- **Never guess.** The forms Ruby resolves heuristically are clean errors instead: `puts -1`, `puts [1]`, `puts (1)` each say _"ambiguous without parens"_ and show both readings, and the same three shapes refuse after a dot call's name — `"x".slice -1`, where Ruby itself warns "ambiguous first argument", is the refusal wearing Ruby's own doubt. `foo - 1` stays subtraction, dotted or bare. And `**` keeps Ruby's answer for the clean cases — `-2 ** 2` is `-4`, a minus before `**` applies last ([ADR 0033](../adr/0033-2026-08-11-exponent-is-starstar-and-pow.md)) — while the one corner where Ruby contradicts itself refuses: `-5.abs ** 2` is `25` in Ruby though `-5 ** 2` is `-25`, the literal fusion winning in one and losing in the other, so Portland shows the menu (`((-5).abs) ** 2` or `-(5.abs ** 2)`) rather than importing a rule nobody can recite or silently answering differently.
