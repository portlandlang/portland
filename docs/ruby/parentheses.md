...
- **Never guess.** The forms Ruby resolves heuristically are clean errors instead: `puts -1`, `puts [1]`, `puts (1)` each say _"ambiguous without parens"_ and show both readings, and the same three shapes refuse after a dot call's name — `"x".slice -1`, where Ruby itself warns "ambiguous first argument", is the refusal wearing Ruby's own doubt. `foo - 1` stays subtraction, dotted or bare. And `-2 ** 2` refuses outright ([ADR 0033](../adr/0033-2026-08-11-exponent-is-starstar-and-pow.md)): Ruby answers `-4`, every spreadsheet answers `4`, and ES2016 made the parenthesis mandatory after watching that split — Portland sides with the refusal, one paren to say which reading you meant.
