...
**Status:** decided ([ADR 0020](../adr/0020-2026-07-23-heredocs-squiggly-only.md)). Built in the seed and the compiler, differentially pinned — see [the language](../language.md#values). Ruby behavior below was verified against Ruby 4.0.6.
...
The terminator can be any identifier — `<<~sql`, `<<~Sql`, `<<~_x`, `<<~q1`, even `<<~puts` all parse — and the closing line must match exactly. `<<~ EOS`, with a space, is a syntax error.

And `<<` does double duty: it is also the shift/append operator. Ruby tells the two apart by asking whether the receiver is a known **local variable** or a **method**:
