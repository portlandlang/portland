...
- **A name nothing defines** (built) — `'SOME_SIGNIFICANT_NUMBER' is not defined`, where Ruby raises `NameError` when the line runs and not before; the typo in a rarely-taken branch that Ruby ships. Declines inside a struct's or module's body, whose own defs a bare name reaches first.
- **A name defined twice** (built, [ADR 0052](../adr/0052-2026-09-24-one-definition-per-name.md)) — `'apply' is already defined — rename one` at the second site, and `'panic' is a builtin — rename yours`; Ruby takes the second silently, and load order decides.
- **A constant bound again** (built, [ADR 0053](../adr/0053-2026-09-24-constants.md)) — `'LIMIT' is a constant — it is bound once`, `'LIMIT' is a constant — a constant cannot be mutable`; Ruby warns and proceeds.
...
The checker reports every refusal at once — one block per refusal with its line beneath, in source order, a count at the end — where Ruby stops at the first `NameError` it happens to reach. It sees a whole program ([ADR 0049](../adr/0049-2026-09-24-the-whole-program-door.md)): an entry file and everything `require_relative` pulls in, each file checked once with the declarations of its requires in view — where Ruby's `require_relative` loads at runtime and checks nothing. A required file's locals never cross; a computed require path is declined. A refusal in a required file names it: `lib.pdx:7 | puts name.knd`.
