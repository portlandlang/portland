...
- [Error handling: results underneath, rescue on top](errors.md) — failures are values, never flights — `!` propagates one marked frame at a time, and a method-level `rescue` catches by pattern what its own body's `!`s passed up.
