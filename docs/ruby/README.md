...
- [Error handling: rescue by pattern, not by ancestry](errors.md) — `begin/rescue` survives, but errors are plain struct values and `rescue` filters with a pattern; the rescue modifier, bare `retry`, and ancestry filtering are gone.
