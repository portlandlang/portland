...
**Arrays** are `[1, "two", [3]]`. **Hashes** are `{"key" => value}`, insertion-ordered, any type of key. `%w[rose city]` builds a word array and `%i[paid pending]` a symbol array, each opening with `[`, `(`, or `{` ([ADR 0051](adr/0051-2026-09-24-the-percent-literal-family.md)), with Ruby's content rules ([ADR 0030](adr/0030-2026-07-27-word-array-contents.md)): a backslash escapes the delimiter in use, balanced delimiters nest, `rose\ city` keeps its space, and whitespace runs split once. The rest of the `%` family refuses by name; `%r` waits on regex.
...
Value equality, definition-ordered fields, capitalized names. Struct bodies take fields first, then methods. Inside a method, bare names resolve locals → the receiver's fields → its own methods → top-level methods, with no-shadow enforced across every layer; `with`, `nil?`, `some?`, and `failure?` are reserved — a struct that could define its own presence or failure predicates could lie to every guard (ADR 0044). `self` is the receiver, and exists for the pass-myself-along case.
