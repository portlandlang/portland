...
Ruby-match, with Ruby's silent nil converted into a handled nil — the standing Portland trick, applied once more. The static strictness B wanted arrives anyway: an unhandled maybe refuses to compile.
...
- The seed's "produced no value" panic family shrinks to the genuinely valueless (`puts`, definitions).
- The trio evaluator's slot conflation ("empty slot reads as nil") stops being a documented crudeness for branches and becomes the spec.
- STAGE0's "Where nil would have been" ledger closes: all four Ruby-nil sites now have Portland answers.
- Migration: `x = if c then a end` and `until`-less `while` idioms compile verbatim with the same meaning; nothing silent changes.
