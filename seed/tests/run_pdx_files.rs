...
fn portland_evaluator_reports_the_seed_wording_on_errors() {
    let cases = [
        ...
        // ADR 0037 (revised same-day): `loop` is a keyword form, and only
        // the `do` block shape exists.
        (
            "loop\n",
...
fn portland_types_dumps_method_returns() {
    ...
    assert_eq!(
        ...
        // Parameters (ADR 0048 step one): a default lends its type, otherwise
        // the body's contract — `shout`, `half`, and `head` are the three
        // demand kinds, a dotted call, an operator, an index.
        "box: [Integer]\ndoubled: [Integer]\nevens: [Integer]\nlabel: String\npicked: String\nmode: Unknown\nunit: Order\nbigger: Order\ndef answer # -> Integer\ndef greet(name: String) # -> String\ndef pick(flag: needs nothing) # -> String\ndef maybe_ran(flag: needs nothing) # -> String?\ndef liar # -> String (annotated Integer)\ndef last_of(values: needs '.each') # -> Unknown\ndef guarded(top: Integer?) # -> Integer\ndef bailed(values: [Integer]) # -> Integer\ndef presence(top: Integer?) # -> Integer\ndef compared(top: Integer?) # -> Integer\ndef matched(top: Integer?) # -> Integer\ndef negated(top: Integer?) # -> Integer\ndef both(top: Integer?, other: Integer?) # -> Integer\ndef safe(words: [String]) # -> String?\ndef unguarded(top: Integer?) # -> Unknown\ndef shout(word: needs '.upcase', '.length') # -> Unknown\ndef half(n: needs '/') # -> Unknown\ndef head(list: needs '[]') # -> Unknown\n"
    );
}
